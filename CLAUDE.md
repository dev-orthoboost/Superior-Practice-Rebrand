...
- **Accessible pink:** the `secondary` / `secondary-container` tokens are **#CD406F** (4.6:1 on white, WCAG AA) as of the 2026-07-15 a11y pass — do NOT revert to the old #D45C84 for any text-bearing or button role. The inline logo SVGs intentionally keep their original fills. Footer body text on navy is `text-navy-100/75`+ (never /40–/70).
- **anime.js v3** (self-hosted `js/anime.min.js`, no CDN) + `js/motion.js` — the shared motion system, loaded on every page:
  - Hero load stagger is **pure CSS** (`#hero [data-reveal]` + `hero-reveal-in` keyframes in `css/input.css`), NOT anime.js — this keeps hero paint off the JS critical path (LCP). Each hero `data-reveal` element carries `style="--ri:N"` for its 110ms stagger slot; **a new hero element needs the next `--ri` number** or it animates at slot 0.
